...
const BREVO_API_ENDPOINT = 'https://tadrib-notify-pay-2.vercel.app/api/subscribe';const BREVO_FORM_EMAIL_FIELD_NAME = 'EMAIL';
...
const courseData = {
    ...
    softskills: { originalPrice: 1700 },
        other: { originalPrice: 199 } // السعر الافتراضي لأي دورة أخرى
};
...
const translations = {
    ar: {
        ...
        PayWithCreditCard: "الدفع بالبطاقة البنكية",
        PayWithCashPlus: "الدفع عبر كاش بلوس",
        CashPlusCodeTitle: "كود الدفع كاش بلوس",
        CashPlusCodeInstructions: "يرجى التوجه إلى أقرب وكالة كاش بلوس وتزويدهم بهذا الكود لإتمام عملية الدفع.",
    },
    fr: {
        ...
        PayWithCreditCard: "Payer par Carte Bancaire",
        PayWithCashPlus: "Payer par CashPlus",
        CashPlusCodeTitle: "Votre code de paiement CashPlus",
        CashPlusCodeInstructions: "Veuillez vous rendre à l'agence CashPlus la plus proche et leur fournir ce code pour finaliser le paiement.",
    },
    en: {
        ...
        PayWithCashPlus: "Pay with CashPlus",
        CashPlusCodeTitle: "Your CashPlus Payment Code",
        CashPlusCodeInstructions: "Please go to the nearest CashPlus agency and provide them with this code to complete the payment.",
    }
};

const bookingConfig = {
    paymentApiUrl: 'https://tadrib-notify-pay-2.vercel.app/api/payment',
    
    // المفتاح العام لـ YouCanPay
    youcanPublicKey: 'pub_5c4db849-75e4-4978-9fb6-44a9926c', // استبدل هذا بمفتاحك الحقيقي للإنتاج
    youcanMode: 'live', // 'live' للإنتاج, 'sandbox' للتجارب
...
// [إضافة] عناصر نوافذ الدفع الجديدة
const paymentMethodModal = document.getElementById('payment-method-modal');
const paymentMethodLoader = document.getElementById('payment-method-loader');
const selectCreditCardBtn = document.getElementById('select-credit-card-btn');
const selectCashPlusBtn = document.getElementById('select-cashplus-btn');
const cashplusCodeModal = document.getElementById('cashplus-code-modal');
const cashplusCodeDisplay = document.getElementById('cashplus-code-display');

// عناصر نافذة حالة الدفع الجديدة
...
async function submitToBrevo(email, lang) {
    ...
    try {
        // الاتصال بالخادم الوكيل بدلاً من Brevo مباشرة
        ...
        // إذا فشل الخادم الوكيل، أظهر خطأ
        if (!response.ok) {
            throw new Error('Subscription API server failed');
        }

        // --- إذا نجح الخادم الوكيل ---
...
function updateGenericFormLanguage(lang) {
    ...
    // [تعديل] توسيع النطاق ليشمل النوافذ الجديدة
    document.querySelectorAll('#contact-modal-generic [data-lang-key], #confirmation-modal-generic [data-lang-key], #payment-method-modal [data-lang-key], #cashplus-code-modal [data-lang-key]').forEach(el => {
...
// دالة عامة لفتح النافذة المنبثقة
window.openContactModalGeneric = function(lang, preselectCourse = null) {
    ...
    // إذا تغيرت اللغة، قم بتحديث كل شيء
    if (currentLang !== lang) {
        switchLang(lang);
    } else {
        // إذا كانت اللغة كما هي، فقط حدث نصوص النافذة المنبثقة
...
// دالة عامة لإغلاق النافذة المنبثقة
window.closeContactModalGeneric = function() {
    if (contactModalGeneric) {
        ...
        populateGenericSelects(currentLang); // إعادة تعبئة النماذج باللغة الحالية
        showStepGeneric(bookingConfig.steps[0]);
        
        // إظهار زر الإرسال (الخطوة 2) مجدداً
        if (submitButtonGeneric && submitButtonGeneric.parentElement) {
            submitButtonGeneric.parentElement.classList.remove('hidden');
        }
        
        // [تعديل] إخفاء نوافذ الدفع
        if(paymentMethodModal) paymentMethodModal.classList.add('modal-hidden');
        if(cashplusCodeModal) cashplusCodeModal.classList.add('modal-hidden');
        resetPaymentMethodModal();

        // إعادة تعيين أزرار التأكيد
...
// إخفاء نافذة التأكيد (عند الضغط على "تعديل")
...
// --- [تعديل] المنطق الجديد: التعامل مع الدفع ---

// 1. عندما يضغط المستخدم على "نعم، تأكيد"
async function onConfirmAndProceedToPayment() {
    ...
    try {
        // [تعديل] لم نعد نطلب التوكن هنا. فقط نفتح نافذة الاختيار.
        // تم تجميع بيانات الحجز مسبقاً في `currentBookingData`
        
        hideConfirmDialogGeneric();
        
        // إعادة تعيين أزرار نافذة التأكيد (حتى تعمل إذا ضغط المستخدم "رجوع")
        if (confirmButtonGeneric) confirmButtonGeneric.disabled = false;
        if (editButtonGeneric) editButtonGeneric.disabled = false;
        if (confirmButtonTextGeneric) {
            confirmButtonTextGeneric.innerText = t['ConfirmButton'];
        ...
        // إظهار نافذة اختيار طريقة الدفع
        if(paymentMethodModal) paymentMethodModal.classList.remove('modal-hidden');

    } catch (error) {
        console.error('Error opening payment selection:', error);
...
// 2. [جديد] عند اختيار "الدفع بالبطاقة البنكية"
async function handleSelectCreditCard() {
    ...
    try {
        ...
        // 2. تجهيز بيانات الدفع (للبطاقة)
        const paymentData = {
            ...currentBookingData, // استخدام البيانات المحفوظة
            paymentMethod: 'credit_card', // تحديد الطريقة
            ...utmData
        };
      
        // 3. طلب توكن الدفع من الخادم
...
// 3. [جديد] عند اختيار "الدفع عبر كاش بلوس"
async function handleSelectCashPlus() {
    showPaymentMethodLoader(true);
    ...
    try {
        ...
        // 2. تجهيز بيانات الدفع (لكاش بلوس)
        const paymentData = {
            ...currentBookingData, // استخدام البيانات المحفوظة
            paymentMethod: 'cashplus', // تحديد الطريقة
            ...utmData
        };
      
        // 3. طلب كود كاش بلوس من الخادم
        const response = await fetch(bookingConfig.paymentApiUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(paymentData)
        });
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || 'Failed to initialize CashPlus payment');
        }
        
        const data = await response.json();
        if (data.result !== 'success' || !data.cashplus_code) {
            throw new Error(data.message || 'Invalid CashPlus code received');
        }
        
        const cashPlusCode = data.cashplus_code;
        ...
        // 5. إظهار نافذة كود كاش بلوس
        if(cashplusCodeDisplay) cashplusCodeDisplay.innerText = cashPlusCode;
        if(cashplusCodeModal) cashplusCodeModal.classList.remove('modal-hidden');
        if(paymentMethodModal) paymentMethodModal.classList.add('modal-hidden'); // إخفاء نافذة الاختيار
        
        showPaymentMethodLoader(false);

    } catch (error) {
        console.error('Error during CashPlus submission:', error);
        alert(`Error: ${error.message}. Please try again.`);
        showPaymentMethodLoader(false);
    }
}

// 4. [جديد] دوال مساعدة لنوافذ الدفع
function showPaymentMethodLoader(isLoading) {
    if (isLoading) {
        if(paymentMethodLoader) paymentMethodLoader.classList.remove('hidden');
        if(selectCreditCardBtn) selectCreditCardBtn.disabled = true;
        if(selectCashPlusBtn) selectCashPlusBtn.disabled = true;
    } else {
        if(paymentMethodLoader) paymentMethodLoader.classList.add('hidden');
        if(selectCreditCardBtn) selectCreditCardBtn.disabled = false;
        if(selectCashPlusBtn) selectCashPlusBtn.disabled = false;
    }
}

function resetPaymentMethodModal() {
    showPaymentMethodLoader(false);
}
...
function closePaymentMethodModal() {
    ...
    if(confirmationModalGeneric) confirmationModalGeneric.classList.remove('modal-hidden');
}

function closeCashPlusCodeModal() {
    if(cashplusCodeModal) cashplusCodeModal.classList.add('modal-hidden');
    // هنا، نعتبر أن المستخدم قد نسخ الكود
    // يمكننا إغلاق كل شيء وإظهار رسالة نجاح عامة
    closeContactModalGeneric();
    
    // إظهار نافذة "النجاح" المؤقتة (لأن الدفع لم يتم بعد، ولكن الحجز تم)
    ...
    if (paymentStatusModal && paymentStatusTitle && paymentStatusMessage && paymentSuccessIcon && paymentFailedIcon) {
        ...
        paymentStatusTitle.innerText = t.CashPlusCodeTitle; // "كود الدفع كاش بلوس"
        paymentStatusMessage.innerText = t.CashPlusCodeInstructions; // "يرجى التوجه..."
        paymentSuccessIcon.classList.remove('hidden'); // إظهار أيقونة النجاح
...
// --- المنطق الجديد: التعامل مع العودة (Callback) ---
function handlePaymentStatusModal() {
    ...
    // فصل الـ hash عن الـ query parameters
    if (hash.includes('?')) {
    ...
    // التحقق من أن الـ hash يحتوي على البيانات المطلوبة
    ...
    // إذا كان hash نجاح ولكن لا يوجد transaction_id، أو إذا لم يكن نجاح أو فشل، فهو غير صالح
    ...
    if (paymentStatusModal && paymentStatusTitle && paymentStatusMessage && paymentSuccessIcon && paymentFailedIcon) {
        ...
        if (hashBase === '#payment-success' && (success === '1' || success === 'true')) {
            ...
            // إرسال حدث GTM (إتمام الشراء)
            pushGtmEvent('purchase', {
                transaction_id: transaction_id,
                // يمكنك إضافة تفاصيل التجارة الإلكترونية هنا إذا كانت متاحة
                ecommerce: {
                    currency: 'MAD'
                },
                payment_method: 'credit_card' // تحديد أن هذا الشراء تم بالبطاقة
            });
...
// دالة مساعدة لتنظيف الـ hash من الـ URL بعد المعالجة
...
// دالة عامة لإغلاق نافذة حالة الدفع
...
function switchLang(lang) {
    ...
    if (langTranslations) {
        ...
        if (metaDesc) {
            metaDesc.content = langTranslations.metaDescriptions;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // معالجة حالة الدفع عند تحميل الصفحة (للعودة)
    ...
    if (confirmButtonGeneric) {
        // [تعديل] تغيير المستمع ليشير إلى الدالة الجديدة
        confirmButtonGeneric.addEventListener('click', onConfirmAndProceedToPayment);
    }
});

window.addEventListener('pageshow', (event) => {
    // معالجة bfcache (عند الضغط على زر "الخلف" في المتصفح)
    if (event.persisted) {
        detectAndSetLanguage();
        // إعادة تعيين أي حالة قديمة
        if (confirmButtonGeneric) confirmButtonGeneric.disabled = false;
        if (editButtonGeneric) editButtonGeneric.disabled = false;
    }
});